...
The Backend runs two background timers in-process: CatalogPublisher (synthesizes and writes config/catalog to the Mediator) and InboxProcessor (drains and archives uploaded records). Neither uses distributed locking or leader election. A second concurrently running instance would race both timers against the same bucket paths.

## Decision

The Backend is deployed as exactly one Fly.io instance (`fly scale count 1`). All clients connect to this one instance's URL. Running exactly one instance is an architectural requirement, not just a cost-saving choice.
